    ...
    try {
        ...
        Client.cmd.on('tiktok', async(data) => {
            if (isLimit(data.sender)) return data.reply(mess.limit) 
                      ...
                      res = await axios.get(`https://api.lolhuman.xyz/api/tiktok?apikey=MiraCans&url=${data.body}`)
                      ppt = res.data.result
                           teks = `*Data berhasil didapatkan!*\n\n*Title* : ${ppt.title}\n*Username* : ${ppt.author.username}\n*nickname* : ${ppt.author.nickname}\n*Keyword:* ${ppt.keywoards}\n*description:* ${ppt.description}\n\n_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
               Client.sendFileFromUrl(data.from, `${ppt.thumbnail}`, 'thumbnail.jpg', teks, data.message)
                Client.sendFileFromUrl(data.from, `${ppt.link}`, 'video.mp4', `Video telah terkirim @${data.sender.split('@')[0]}`, data.message)
        })
